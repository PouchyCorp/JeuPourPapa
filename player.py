...
class Player:
    def __init__(self, x, y):
        ...
        self.surf = self.anim.get_frame() ; self.anim.reset_frame()
    ...
    def handle_movement_inputs(self):
        ...
        if keys[pg.K_LEFT]: self.rect.x -= 5; self.moving = True; self.facing_left = True
        if keys[pg.K_RIGHT]: self.rect.x += 5; self.moving = True; self.facing_left = False
        if keys[pg.K_UP]: self.rect.y -= 5; self.moving = True
        if keys[pg.K_DOWN]: self.rect.y += 5; self.moving = True
